...
from .metrics import bbox_iou, piou, probiou
...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        """Compute PIoU2 and DFL losses for bounding boxes."""
        device = pred_bboxes.device
        if fg_mask.sum() == 0:
            return torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)

        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
        loss_piou = piou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, PIoU2=True)
        loss_iou = (loss_piou * weight).sum() / (target_scores_sum + 1e-7)

        # DFL loss
        if self.dfl_loss:
            target_ltrb = bbox2dist(anchor_points, target_bboxes, self.dfl_loss.reg_max - 1)
            loss_dfl = self.dfl_loss(pred_dist[fg_mask].view(-1, self.dfl_loss.reg_max), target_ltrb[fg_mask]) * weight
            loss_dfl = loss_dfl.sum() / (target_scores_sum + 1e-7)
        else:
